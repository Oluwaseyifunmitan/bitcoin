...
const decodeBitcoinTransaction = require('./decode');

const test = ('Decode Bitcoin transaction', () => {
    ...
    const decodedTx = decodeBitcoinTransaction(rawTxHex);

    expect(decodedTx).toBeDefined();
    expect(decodedTx).toHaveProperty('ins');
    expect(decodedTx).toHaveProperty('outs');
    expect(decodedTx).toHaveProperty('version');
    expect(decodedTx).toHaveProperty('locktime');
